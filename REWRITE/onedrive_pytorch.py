...
actual_prices_for_test = test_data_processed[
    "Close"
].values  # Keep original prices for RMSE
...
def objective(
    ...
    if training_end_date in data_full_processed.index:
        ...
        training_data_slice = data_full_processed[
            data_full_processed.index < test_start_fixed
        ]

    # Scale using the global scaler
    scaled_training_data_for_trial = scaler.transform(
        training_data_slice[features].values
    )
    ...
    total_dataset_for_ai_inputs = pd.concat(
        (data_full_processed[features], test_data_processed[features]), axis=0
    )

    # Slice to get exactly [test_start - prediction_days, end]
    ai_inputs_raw_slice = total_dataset_for_ai_inputs[
        len(total_dataset_for_ai_inputs) - len(test_data_processed) - prediction_days :
    ].values
