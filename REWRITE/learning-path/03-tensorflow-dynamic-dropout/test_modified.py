...
# RUN-DOWN: Lines 63 to 89 set up a 3-layer neural network. First, the AI generates its first initial prediction based on the first 60 days of data. It then drops 20% of the neurons to avoid creating a bias in the ai model. 
...
# The whole reason we drop 20% of the neurons to avoid overfitting is that if we don't, the ai model might just memorize the training data instead of learning to generalize from it.
...
ai_inputs = scaler.fit_transform(ai_inputs) # NOTE: use scaler.transform here to reuse the scaler fitted on training data. Current code calls fit_transform which refits the scaler on combined data.
...
graph.set_title(f'{crypto_currency} price prediction')
graph.set_xlabel('Date')
graph.set_ylabel(f'{crypto_currency} price')
