...
# Add this function near the top of file, after imports:
...
# Then modify model definition section. Replace the current LSTM/Dropout pattern with:
epochs = 20  # Match this with model.fit epochs parameter
...
# Add the dynamic dropout callback to training
...
print(f"To view TensorBoard, run this command in terminal:")
print(f"tensorboard --logdir=logs/fit")
print(f"Then open http://localhost:6006 in browser")
print(f"{'='*60}\n")

ai.compile(optimizer='adam', loss='mean_squared_error')

# Modify model.fit call to include the new callback:
